...
ui.page_opts(title="PyShiny App with Plot MhamedM", fillable=True)
...
with ui.sidebar():
    ...
    # An integer representing the minimum number of bins (10).
    # An integer representing the maximum number of bins (100).
    # An integer representing the initial value of the slider (40).
    # A string id ("selected_number_of_bins") that uniquely identifies this input value. 
    ui.input_slider("selected_number_of_bins", "Number of Bins", 10, 100, 40)

# Define a plot function for rendering a histogram 
@render.plot(alt="Histogram (distribution)")
def histogram():
    # Define the number of points to generate
    count_of_points: int = 200
...
@render.plot(alt="Heatmap (distribution)")
def heatmap():
    # Define the number of points to generate
